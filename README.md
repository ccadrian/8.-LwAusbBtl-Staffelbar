...
| **Barcode** | Jedes Getränk kann seinen Strichcode bekommen: einmal scannen und zuordnen, danach erkennt die App die Flasche. Scannen geht auf der Startseite (zählt das Getränk direkt nach), in der Zählrunde (springt zum Getränk), beim Einkauf nachtragen und in der Getränkemaske. Ein unbekannter Code fragt einmal, zu welchem Getränk er gehört – und holt dabei den Produktnamen aus **Open Food Facts** (frei, ohne Schlüssel): „Paulaner Spezi Zero 0,5 l“ steht dann schon im Feld, samt Kategorie. Ist die Flasche dort nicht bekannt, tippt man den Namen. Ohne Kamera lässt sich der Code von Hand eintippen. |
